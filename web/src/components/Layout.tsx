  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <header className="sticky top-0 z-10 border-b border-border bg-surface px-4 py-3">
        ...
        {/* Mobile nav dropdown */}
        {mobileMenuOpen && (
          <nav className="mx-auto mt-2 flex max-w-[1180px] flex-col gap-1 border-t border-border pt-2 md:hidden" aria-label="Mobile navigation">
            <NavLink to="/deals" onClick={() => setMobileMenuOpen(false)}>Deals</NavLink>
            <NavLink to="/about" onClick={() => setMobileMenuOpen(false)}>About</NavLink>
          </nav>
        )}
      </header>
      <main id="main-content" className="mx-auto w-full max-w-[1180px] flex-1 p-4" onClick={() => setMobileMenuOpen(false)}>
        <Outlet />
      </main>
