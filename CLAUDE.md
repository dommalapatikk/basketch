...
- **One paid service, capped; everything else free.** GitHub Actions free tier, Supabase free 500 MB, Vercel Hobby, and Gemini on the **free tier** (the Google project behind `GOOGLE_AI_API_KEY` must have **no billing account**). The **only** paid dependency is **OpenRouter** (`openai/gpt-5-nano`, the classification *judge*), approved by the PM on **2026-09-15**, capped at **USD 5 per month**. The cap is enforced three ways: (1) the CI key's own OpenRouter credit limit with a **monthly** reset and auto top-up off — a PM-owned setting the code reads at run start and refuses to spend without, including refusing a limit that resets on any other cadence (a USD 5 *daily* limit would permit ~USD 150 a month); (2) `SpendLedger` in `transformation/domain/spend.ts` reserves a call's worst-case cost before it is made and settles the actual cost after, so an unknown outcome is charged at the worst case, never at zero; (3) every paid call sends an explicit `max_tokens`. If the allowance cannot be read, the run continues **without the judge** rather than guessing (AP-10). Adding a paid service, or raising the cap, is a PM decision recorded in an ADR. **No paid unblocking proxy, ever** — that is a legal line, not a budget one (see "Never circumvent" above).
