# Tests for aktionis.ch scraper: normalization, HTML parsing, HTTP mocking, and edge cases.
...
from fetch import fetch_coop_deals, fetch_store_deals, fetch_page
from normalize import (
    calculate_discount_percent,
    normalize_aktionis_deal,
    normalize_coop_deal,
...
class TestNormalizeAktionisDeal:
    def test_valid_deal_with_store_name(self) -> None:
        raw = {
        ...
        result = normalize_aktionis_deal(raw, "aldi")
        assert result is not None
    ...
    def test_store_name_is_used_as_store_field(self) -> None:
        raw = {
            "name": "Test Product",
            "sale_price": 5.0,
            "original_price": None,
        ...
        for store_name in ("coop", "lidl", "aldi", "migros"):
            result = normalize_aktionis_deal(raw, store_name)
            assert result is not None
            assert result["store"] == store_name

    def test_missing_name(self) -> None:
        assert normalize_aktionis_deal({"sale_price": 10.0}, "coop") is None

    def test_missing_sale_price(self) -> None:
        assert normalize_aktionis_deal({"name": "Test", "sale_price": None}, "coop") is None

    def test_zero_sale_price(self) -> None:
        assert normalize_aktionis_deal({"name": "Test", "sale_price": 0.0}, "coop") is None
    ...
    def test_no_original_price_calculates_no_discount(self) -> None:
        ...
        result = normalize_aktionis_deal(raw, "coop")
        assert result is not None
    ...
    def test_camel_case_keys(self) -> None:
        ...
        result = normalize_aktionis_deal(raw, "lidl")
        assert result is not None
        expected_keys = {
            "store",
            "productName",
...
class TestNormalizeCoopDeal:
    """Tests for deprecated normalize_coop_deal alias — verifies it delegates correctly."""
...
class TestEndToEnd:
    def test_fixture_produces_deals(self) -> None:
        ...
        for card in cards:
            ...
            if raw:
                deal = normalize_aktionis_deal(raw, "coop")
                if deal:
                    deals.append(deal)
    ...
    def test_fixture_produces_deals_with_custom_store(self) -> None:
        """Parse fixture HTML and confirm store field reflects the given store_name."""
        html_path = FIXTURES_DIR / "coop-page-1.html"
        soup = BeautifulSoup(html_path.read_text(), "lxml")
        cards = soup.select("div.card.dealtype-deal")
        ...
        for card in cards:
            raw = parse_deal_card(card)
            if raw:
                deal = normalize_aktionis_deal(raw, "aldi")
                if deal:
                    deals.append(deal)

        assert len(deals) > 10
        for deal in deals:
            assert deal["store"] == "aldi"


# ── HTTP layer tests (mocked) ─────────────────────────────────────


COOP_BASE_URL = "https://aktionis.ch/vendors/coop"
ALDI_BASE_URL = "https://aktionis.ch/vendors/aldi-suisse"
...
class TestFetchPage:
    def test_happy_path_returns_deals(self) -> None:
        ...
        with patch("fetch.requests.get", return_value=mock_response) as mock_get:
            result = fetch_page(1, COOP_BASE_URL)
    ...
    def test_uses_provided_base_url(self) -> None:
        """fetch_page should construct the URL from the provided base_url."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.text = "<html><body></body></html>"

        with patch("fetch.requests.get", return_value=mock_response) as mock_get:
            fetch_page(3, ALDI_BASE_URL)
            called_url = mock_get.call_args[0][0]
            assert called_url == f"{ALDI_BASE_URL}/3"


class TestFetchStoreDeals:
    def test_happy_path_paginates(self) -> None:
        """fetch_store_deals should paginate until empty page."""
        html_path = FIXTURES_DIR / "coop-page-1.html"
        ...
        with patch("fetch.requests.get", side_effect=mock_get):
            deals = fetch_store_deals("coop", "coop", max_pages=5)
            assert len(deals) > 0
            # Should have stopped after page 2 (empty)
            assert call_count == 2

    def test_store_name_propagated_to_deals(self) -> None:
        """All deals should carry the store_name passed in."""
        html_path = FIXTURES_DIR / "coop-page-1.html"
        fixture_html = html_path.read_text()
        ...
        with patch(
            ...
            deals = fetch_store_deals("aldi-suisse", "aldi", max_pages=5)
            assert len(deals) > 0
            for deal in deals:
                assert deal["store"] == "aldi"
    ...
    def test_network_failure_returns_empty_list(self) -> None:
        ...
        with patch(
            ...
            deals = fetch_store_deals("lidl", "lidl", max_pages=3)
            assert deals == []

    def test_all_deals_have_camel_case_keys(self) -> None:
        """Every deal from fetch_store_deals must use camelCase keys."""
        html_path = FIXTURES_DIR / "coop-page-1.html"
        fixture_html = html_path.read_text()
    ...
    def test_zero_deals_is_success(self) -> None:
        ...
        with patch("fetch.requests.get", return_value=empty_response):
            deals = fetch_store_deals("coop", "coop", max_pages=1)
            assert deals == []


class TestFetchCoopDeals:
    """Tests for deprecated fetch_coop_deals alias — verifies it delegates correctly."""
...
class TestMain:
    def test_main_stdout(self) -> None:
        ...
        with (
            ...
            patch("sys.argv", ["main.py", "coop", "coop"]),
        ):
            exit_code = main()
            assert exit_code == 0
    ...
    def test_main_file_output(self, tmp_path: Path) -> None:
        ...
        with (
            patch("fetch.requests.get", return_value=empty_response),
            patch("sys.argv", ["main.py", "aldi-suisse", "aldi", str(output_file)]),
        ):
            exit_code = main()
            assert exit_code == 0
    ...
    def test_main_missing_args_returns_1(self) -> None:
        """main() should return exit code 1 when store args are missing."""
        from main import main
    ...
    def test_main_only_slug_returns_1(self) -> None:
        """main() should return exit code 1 when store-name arg is missing."""
        from main import main

        with patch("sys.argv", ["main.py", "coop"]):
            exit_code = main()
            assert exit_code == 1
